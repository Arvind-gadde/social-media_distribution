...
import { register, getGoogleUrl } from "../api/auth";
...
export default function RegisterPage() {
  ...
  const field = (id: keyof FormState, label: string, type = "text", placeholder = "") => (
    <div>
      ...
      <input
        id={id} type={type} autoComplete={type === "password" ? "new-password" : id}
        placeholder={placeholder} value={form[id]}
        onChange={(e) => { setForm((f) => ({ ...f, [id]: e.target.value })); if (errors[id]) setErrors((er) => ({ ...er, [id]: undefined })); }}
        className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 transition ${errors[id] ? "border-red-400 bg-red-50" : "border-slate-200 bg-white"}`}
      />
      ...
      <div className="w-full max-w-md">
        <div className="card p-8 space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500 text-sm">Start distributing to 12+ platforms</p>
          </div>
